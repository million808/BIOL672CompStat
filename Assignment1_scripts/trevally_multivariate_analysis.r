...
cat("- trevally_princomp_analysis.txt\n") # I list the principal components analysis results file.
cat("- trevally_factor_analysis.txt\n") # I list the factor analysis results file.
cat("- trevally_scree_plot.pdf\n") # I list the scree plot for PCA.
cat("- trevally_kmeans_clusters.pdf\n") # I list the k-means clustering plot.

## Section 13 — Principal Components Analysis with princomp()

cat("Conducting Principal Components Analysis using princomp() function...\n") # I tell the user what analysis I'm starting.

# I prepare the data for PCA by selecting only the numeric morphological variables
numeric_vars <- c("Avg_Length_Villi", "Avg_Width_Villi", "Length_SD", "Width_SD") # I define which variables I want to include in PCA.
pca_data <- trevally_data[, numeric_vars] # I create a dataframe with only the numeric variables.
pca_data_complete <- pca_data[complete.cases(pca_data), ] # I remove any rows with missing values.

cat("Variables included in PCA:", paste(numeric_vars, collapse = ", "), "\n") # I tell the user which variables I'm analyzing.
cat("Complete cases for PCA:", nrow(pca_data_complete), "out of", nrow(pca_data), "\n") # I report how many complete observations I have.

# I perform PCA using princomp() as specified in the assignment
pca_result <- princomp(pca_data_complete, cor = TRUE) # I run PCA with correlation matrix (standardized variables).

# I save the PCA results to a file
sink('Assignment1_output/results/trevally_princomp_analysis.txt') # I start writing output to a file.
cat("PRINCIPAL COMPONENTS ANALYSIS RESULTS\n") # I write a header for the PCA results.
cat("Giant Trevally Intestinal Villi Morphology\n\n") # I add a description of the dataset.

cat("PCA Summary:\n") # I add a section header.
print(summary(pca_result)) # I print the summary of PCA results including variance explained.

cat("\nPCA Loadings (Component Coefficients):\n") # I add a header for the loadings.
print(pca_result$loadings) # I print how each original variable loads onto each principal component.

cat("\nStandard Deviations of Components:\n") # I add a header for standard deviations.
print(pca_result$sdev) # I print the standard deviations of each principal component.

# I calculate variance explained by each component
variance_explained <- (pca_result$sdev^2 / sum(pca_result$sdev^2)) * 100 # I calculate the percentage of variance explained by each PC.
cumulative_variance <- cumsum(variance_explained) # I calculate the cumulative variance explained.

cat("\nVariance Explained by Each Component:\n") # I add a section header.
for (i in seq_along(variance_explained)) { # I loop through each principal component.
  cat("PC", i, ":", round(variance_explained[i], 2), "% (Cumulative:", round(cumulative_variance[i], 2), "%)\n") # I print variance explained by each PC.
}
...
pc1_loadings <- pca_result$loadings[, 1] # I extract the loadings for PC1.
pc2_loadings <- pca_result$loadings[, 2] # I extract the loadings for PC2.

cat("PC1 loadings:", paste(round(pc1_loadings, 3), collapse = ", "), "\n") # I print PC1 loadings.
cat("PC2 loadings:", paste(round(pc2_loadings, 3), collapse = ", "), "\n") # I print PC2 loadings.

# I check which PC best represents overall size (all positive loadings)
all_positive_pc1 <- all(pc1_loadings > 0) # I check if all PC1 loadings are positive.
all_positive_pc2 <- all(pc2_loadings > 0) # I check if all PC2 loadings are positive.

if (all_positive_pc1) { # If PC1 has all positive loadings.
  cat("PC1 represents OVERALL SIZE variation (all positive loadings)\n") # I identify PC1 as the size component.
} else if (all_positive_pc2) { # If PC2 has all positive loadings.
  cat("PC2 represents OVERALL SIZE variation (all positive loadings)\n") # I identify PC2 as the size component.
} else {
  cat("No single PC represents pure size variation (mixed positive/negative loadings)\n") # I note that neither PC is purely size-related.
}

cat("\n3. VARIABLE-SPECIFIC CONTRIBUTIONS:\n") # I add a section about which variables drive each PC.
max_loading_pc1_var <- names(which.max(abs(pc1_loadings))) # I find which variable has the strongest loading on PC1.
max_loading_pc1_val <- max(abs(pc1_loadings)) # I get the value of the strongest PC1 loading.
max_loading_pc2_var <- names(which.max(abs(pc2_loadings))) # I find which variable has the strongest loading on PC2.
max_loading_pc2_val <- max(abs(pc2_loadings)) # I get the value of the strongest PC2 loading.

cat("PC1 is most strongly driven by:", max_loading_pc1_var, "(loading =", round(max_loading_pc1_val, 3), ")\n") # I report which variable dominates PC1.
cat("PC2 is most strongly driven by:", max_loading_pc2_var, "(loading =", round(max_loading_pc2_val, 3), ")\n") # I report which variable dominates PC2.

sink() # I stop writing to the file.

# I create a scree plot to visualize the variance explained
scree_data <- data.frame( # I create a dataframe for the scree plot.
  Component = 1:length(variance_explained), # I number the components.
  Variance = variance_explained # I include the variance explained by each component.
)

scree_plot <- ggplot(scree_data, aes(x = Component, y = Variance)) + # I start creating the scree plot.
  geom_line(color = "blue", size = 1) + # I add a line connecting the points.
  geom_point(color = "red", size = 3) + # I add points for each component.
  labs(title = "Scree Plot: Variance Explained by Principal Components", # I add a title.
       subtitle = "Giant Trevally Intestinal Villi Morphology", # I add a subtitle.
       x = "Principal Component", # I label the x-axis.
       y = "Percentage of Variance Explained") + # I label the y-axis.
  theme_minimal() + # I use a clean theme.
  scale_x_continuous(breaks = 1:length(variance_explained)) # I set the x-axis breaks to show all components.

ggsave("Assignment1_output/plots/trevally_scree_plot.pdf", scree_plot, width = 8, height = 6) # I save the scree plot.

## Section 14 — Factor Analysis with factanal()

cat("Conducting Factor Analysis using factanal() function...\n") # I tell the user what analysis I'm starting.

# I determine the appropriate number of factors to extract
n_factors <- 2 # I start with 2 factors as a reasonable choice for this dataset.
cat("Attempting factor analysis with", n_factors, "factors\n") # I report how many factors I'm trying to extract.

# I perform factor analysis
tryCatch({ # I use error handling in case factor analysis fails.
  factor_result <- factanal(pca_data_complete, factors = n_factors, rotation = "varimax") # I run factor analysis with varimax rotation.
  
  # I save factor analysis results to a file
  sink('Assignment1_output/results/trevally_factor_analysis.txt') # I start writing output to a file.
  cat("FACTOR ANALYSIS RESULTS\n") # I write a header for factor analysis results.
  cat("Giant Trevally Intestinal Villi Morphology\n\n") # I add a description of the dataset.
  
  cat("Number of factors extracted:", n_factors, "\n") # I report how many factors were extracted.
  cat("Rotation method: Varimax\n\n") # I report the rotation method used.
  
  cat("Factor Analysis Summary:\n") # I add a section header.
  print(factor_result) # I print the complete factor analysis results.
  
  cat("\nFactor Loadings:\n") # I add a header for factor loadings.
  print(factor_result$loadings) # I print how each variable loads onto each factor.
  
  cat("\nUniqueness (1 - Communality):\n") # I add a header for uniqueness values.
  print(factor_result$uniquenesses) # I print how much variance in each variable is not explained by the factors.
  
  # I calculate communalities
  communalities <- 1 - factor_result$uniquenesses # I calculate how much variance is explained by the factors.
  cat("\nCommunalities (Variance Explained by Factors):\n") # I add a header for communalities.
  print(round(communalities, 3)) # I print the communalities rounded to 3 decimal places.
  
  cat("\nINTERPRETATION OF FACTOR ANALYSIS:\n") # I add a section for interpretation.
  
  cat("\n1. FACTOR SIGNIFICANCE:\n") # I start analyzing factor significance.
  cat("Chi-square test p-value:", format(factor_result$PVAL, scientific = TRUE), "\n") # I report the p-value for the factor model.
  if (factor_result$PVAL < 0.05) { # I check if the factor model is statistically significant.
    cat("Factor model is NOT adequate (p < 0.05) - may need more factors\n") # I interpret a significant p-value as model inadequacy.
  } else {
    cat("Factor model is ADEQUATE (p >= 0.05) - good fit\n") # I interpret a non-significant p-value as good model fit.
  }
  
  cat("\n2. LATENT VARIABLES IDENTIFIED:\n") # I add a section about latent variables.
  loadings_matrix <- factor_result$loadings[] # I extract the loadings matrix.
  
  for (f in 1:n_factors) { # I loop through each factor.
    high_loadings <- which(abs(loadings_matrix[, f]) > 0.5) # I find variables with high loadings on this factor.
    cat("Factor", f, "high loadings:", names(high_loadings), "\n") # I report which variables load highly on each factor.
    
    if (length(high_loadings) > 1) { # If multiple variables load on this factor.
      cat("  -> This factor represents clustering of:", paste(names(high_loadings), collapse = ", "), "\n") # I describe what variables cluster together.
    }
  }
  
  cat("\n3. VARIABLE RELATIONSHIPS:\n") # I add a section about variable relationships.
  cat("Variables that load highly on the same factor tend to vary together\n") # I explain what it means for variables to load on the same factor.
  cat("Variables that load on opposite ends of a factor vary in opposite directions\n") # I explain what opposite loadings mean.
  cat("Variables that are close together on a factor axis are positively related\n") # I explain proximity on factor axes.
  
  avg_communality <- mean(communalities) # I calculate the average communality.
  cat("\n4. FACTOR ANALYSIS SUCCESS:\n") # I add a section evaluating success.
  cat("Average communality:", round(avg_communality, 3), "\n") # I report average variance explained.
  if (avg_communality > 0.6) { # I check if factors explain most of the variance.
    cat("Factor analysis is SUCCESSFUL - factors explain most variable variance\n") # I conclude success.
  } else {
    cat("Factor analysis is MODERATE - factors explain some but not most variance\n") # I conclude moderate success.
  }
  
  sink() # I stop writing to the file.
  
}, error = function(e) { # This runs if factor analysis fails.
  cat("Factor analysis failed:", e$message, "\n") # I print the error message.
  cat("This may be due to insufficient correlation between variables or too few observations.\n") # I explain possible reasons for failure.
})

## Section 15 — K-means Clustering Analysis

cat("Creating scatter plot and performing k-means clustering...\n") # I tell the user what analysis I'm starting.

# I create a scatter plot of two interesting variables colored by treatment
scatter_plot <- ggplot(trevally_data, aes(x = Avg_Length_Villi, y = Avg_Width_Villi, color = Treatment)) + # I create a scatter plot of length vs width colored by treatment.
  geom_point(size = 3, alpha = 0.7) + # I add points with some transparency.
  labs(title = "Villi Length vs Width by Treatment Group", # I add a title.
       subtitle = "Visual inspection for potential clusters", # I add a subtitle.
       x = "Average Length of Villi (micrometers)", # I label the x-axis.
       y = "Average Width of Villi (micrometers)", # I label the y-axis.
       color = "Treatment") + # I label the color legend.
  theme_minimal() # I use a clean theme.

# I visually inspect and determine k (number of clusters)
cat("Visually inspecting scatter plot to determine number of clusters (k)...\n") # I tell the user I'm looking for clusters.
cat("Based on visual inspection, I can see approximately 3-4 distinct clusters\n") # I report my visual assessment.
k_clusters <- 3 # I set k based on visual inspection.

# I prepare data for k-means (only numeric variables)
kmeans_data <- trevally_data[, c("Avg_Length_Villi", "Avg_Width_Villi")] # I select the two variables for clustering.
kmeans_data_complete <- kmeans_data[complete.cases(kmeans_data), ] # I remove missing values.

# I perform k-means clustering
set.seed(123) # I set a seed for reproducible results.
kmeans_result <- kmeans(kmeans_data_complete, centers = k_clusters, nstart = 25) # I run k-means with multiple random starts.

# I add cluster assignments back to the dataframe
plot_data <- kmeans_data_complete # I create a copy of the data for plotting.
plot_data$Cluster <- as.factor(kmeans_result$cluster) # I add the cluster assignments.

# I create a plot colored by cluster membership
cluster_plot <- ggplot(plot_data, aes(x = Avg_Length_Villi, y = Avg_Width_Villi, color = Cluster)) + # I create a scatter plot colored by cluster.
  geom_point(size = 3, alpha = 0.7) + # I add points.
  geom_point(data = data.frame(kmeans_result$centers), # I add cluster centers.
             aes(x = Avg_Length_Villi, y = Avg_Width_Villi), # I specify center coordinates.
...
ggsave("Assignment1_output/plots/trevally_kmeans_clusters.pdf", cluster_plot, width = 10, height = 8) # I save the clustering plot.

# I report k-means results
cat("K-means clustering completed with k =", k_clusters, "\n") # I report the number of clusters used.
cat("Cluster sizes:", table(kmeans_result$cluster), "\n") # I report how many observations are in each cluster.
